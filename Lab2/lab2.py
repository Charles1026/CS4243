...
def make_gaussian_kernel(ksize, sigma):
    ...
    mean = math.floor(ksize / 2) if (ksize % 2 == 1) else ksize / 2
    
    for i in range(ksize):
        for j in range(ksize):
            kernel[i, j] = math.exp((((i - mean) ** 2) + ((j - mean) ** 2)) / (-2 * (sigma ** 2)))
...
def non_maximum_suppression_interpol(d_mag, d_angle, display=True):
    ...
    # YOUR CODE HERE
    def getCellValue(row: int, col: int) -> float:
        return d_mag[row, col] if (0 <= row < d_mag.shape[0] and 0 <= col < d_mag.shape[1]) else 0

  
    def suppress(row: int, col: int):
        # get hypothenuse length
        orientation = d_angle[row, col]
        ratio = 0
        # ratio is adj
        if (math.pi / 4) <= abs(orientation) <= ((3 * math.pi) / 4):
            ratio = abs(1 / math.tan(orientation))
        # ratio is opp
        else:
            ratio = abs(math.tan(orientation))

        # Get the correct quadrant
        xValue = math.cos(orientation)
        yValue = math.sin(orientation)
        quadrant = (int(math.copysign(1, xValue)), int(math.copysign(1, yValue)))
        
        firstNeighbor = 0
        secondNeighbor = 0
        
        # Closer to the x axis
        if abs(xValue) > (math.sqrt(2) / 2):
            firstNeighbor = getCellValue(row + quadrant[0], col)
            secondNeighbor = getCellValue(row - quadrant[0], col)
        # Closer to y axis
        else:
            firstNeighbor = getCellValue(row, col + quadrant[1])
            secondNeighbor = getCellValue(row, col - quadrant[1])
            
        # Interpolate with diagonals
        firstNeighbor  += ratio * (getCellValue(row + quadrant[0], col + quadrant[1]) - firstNeighbor)
        secondNeighbor += ratio * (getCellValue(row - quadrant[0], col - quadrant[1]) - secondNeighbor)
        
        # Suppress if non maxima
        magnitude = getCellValue(row, col)  
        return magnitude if magnitude > firstNeighbor and magnitude > secondNeighbor else 0
    
    for i in range(d_mag.shape[0]):
        for j in range(d_mag.shape[1]):
            out[i, j] = suppress(i, j)
...
def non_maximum_suppression(d_mag, d_angle, display=True):
    ...
    # YOUR CODE HERE
    def getTransform(angle: float):
        if (-22.5 <= angle < 22.5 or 157.5 <= angle or -157.5 > angle):
            return (1, 0)
    ...
    def suppress(row: int, col: int):
        transform = getTransform(d_angle_180[row, col])   
        firstNeighbour: float = getCellValue(row + transform[0], col + transform[1])
        secondNeighbour: float = getCellValue(row - transform[0], col - transform[1])

        value = d_mag[row, col]
        return value if value > firstNeighbour and value > secondNeighbour else 0
...
def double_thresholding(inp, perc_weak=0.1, perc_strong=0.3, display=True):
    ...
    # YOUR CODE HERE
    maxMagnitude: float = np.amax(inp)
    minMagnitude: float = np.amin(inp)
    
    maxRange = maxMagnitude - minMagnitude
    highThreshold = minMagnitude + (perc_strong * maxRange)
    lowThreshold  = minMagnitude + (perc_weak   * maxRange)
    
    strong_edges = (inp > highThreshold).astype(int)
    weak_edges = ((lowThreshold < inp) & (inp < highThreshold)).astype(int)
...
def edge_linking(weak, strong, n=200, display=True):
    ...
    for iteration in range(n):
        hasUpdates = False
        weakElementCoords = np.argwhere(weak)
        for coord in weakElementCoords:
            i = coord[0]
            j = coord[1]
            surroundingValuesSummed = np.sum(strong[i - 1 : i + 2, j - 1 : j + 2])
            if surroundingValuesSummed > 0:
                hasUpdates |= True
                strong[i, j] = weak[i, j]
                weak[i, j] = 0
        if not hasUpdates:
...
def hough_vote_lines(img):
    ...
    '''
    # YOUR CODE HERE
    def solveForP(coord, theta: float):
        return coord[0] * math.cos(theta) + coord[1] * math.sin(theta)
    
    pMax = math.ceil(math.hypot(img.shape[0], img.shape[1]))
    pCellWidth = 1
    distances = np.arange(-pMax, pMax, pCellWidth)
    
    thetaCellWidth = math.pi / 180
    thetas = np.arange(0, math.pi, thetaCellWidth)
    
    # Add 1 to P cells to account for largest p as we always round down
    A = np.zeros((len(distances), len(thetas)), dtype = int)

    edgeCoords = np.argwhere(img)
    
    for coord in edgeCoords:
        for thetaIndex in range(len(thetas)):
            theta = thetaIndex * thetaCellWidth
            p = solveForP(coord, theta)
            pIndex = math.floor((p + pMax) / pCellWidth)

            A[pIndex, thetaIndex] += 1
...
def hough_vote_circles(img, radius = None):
    ...
    #   Remember padding
    radiusStep = xStep = yStep = 1
    
    R = np.arange(math.ceil(R_min), math.ceil(R_max), radiusStep)
    X = np.arange(0, h, xStep)
    Y = np.arange(0, w, yStep)
    
    A = np.zeros((R.shape[0], X.shape[0], Y.shape[0]), dtype = int)
    ...
    for radius in R:
        ...
        nonZeroPoints = rr.shape[0]
        
        #3.3 For each edge point:
        for coord in edgeCoords:
            # Center the mask over that point and update the accumulator array
            centeredRR = rr + coord[0]
            centeredCC = cc + coord[1]
            circleCoords = zip(centeredRR, centeredCC)
            clippedCoords = circleCoords[(circleCoords[0] >= X[0]) & (circleCoords[0] <= X[-1]) & (circleCoords[1] >= Y[0]) & (circleCoords[1] <= Y[-1])]
            for coord in clippedCoords:
                A[coord[0], coord[1]] = 1
    # END
